...
19_unions: $(SDIR)/19_unions.c
	$(CC) -o $(DDIR)/$@ $^ $(CFLAGS)
	strip -s $(DDIR)/$@.exe
	$(DDIR)/$@

20_pointer-arithmetic: $(SDIR)/20_pointer-arithmetic.c
	$(CC) -o $(DDIR)/$@ $^ $(CFLAGS)
	strip -s $(DDIR)/$@.exe
	$(DDIR)/$@

21_function-pointers: $(SDIR)/21_function-pointers.c
	$(CC) -o $(DDIR)/$@ $^ $(CFLAGS)
	strip -s $(DDIR)/$@.exe
	$(DDIR)/$@

.PHONY: 01_hello-world 02_vars-types 03_arrays 04_multi-arrays 05_conditions 06_strings 07_for-loops 08_while-loops 09_functions 10_static 11_pointers 12_structs 13_function-args 14_dynamic-allocation 15_arrays-and-pointers 16_recursion 17_linked-lists 18_binary-trees 19_unions 20_pointer-arithmetic 21_function-pointers
